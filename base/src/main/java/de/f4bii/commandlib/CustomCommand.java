...
import de.f4bii.commandlib.exception.ExceptionHandlerException;
...
//        commands.sort(SubCommandObject::compareTo);
    ...
    public final void execute(S sender, String[] args) throws Throwable {
        try {
            ...
            if (!handleException(ex, sender)) throw ex;
        }
    }
    ...
    private void processCommandExecution(S sender, String[] args) throws Throwable {
        ...
        if (!object.matches(args))
            throw new CommandNotFoundException();
        if (!object.enoughParameters(args)) {
            wrongCommandUsage(args);
            return;
        }
    ...
    private SubCommandObject<S> getCommandMatchingArguments(S sender, String[] arg) {
        List<SubCommandObject<S>> collect = commands.stream().sorted((o1, o2) -> o2.getMatchingSize(arg) - o1.getMatchingSize(arg))/*.filter(sSubCommandObject -> sSubCommandObject.matches(arg))*/
                .filter(sSubCommandObject -> sSubCommandObject.canExecute(sender)).collect(Collectors.toList());
        Optional<SubCommandObject<S>> first = collect.stream().findFirst();
        return first.orElseThrow(CommandNotFoundException::new);
    }

    protected void wrongCommandUsage(String[] args) {
        // to be overwritten
    }
     ...
     */
    protected final boolean handleException(Throwable exception, S sender) {
        MethodHandle method = exceptions.get(exception.getClass());
        if (method == null) return false;
        try {
            method.invoke(this, sender, exception);
        } catch (Throwable e) {
            throw new ExceptionHandlerException("This is a serius error. Please create a Github Issue on https://github.com/F4bii/CommandLib with the errormessage and some more Information", e);
        }
        return true;
    }
}
