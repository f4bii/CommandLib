...
//            return false;
        for (int i = 0; i < args.length && i < cmdArgs.length; i++) {
            if (isCustomVariable(cmdArgs[i]))
                continue;
    ...
    public int getMatchingSize(String[] args) {
        int count = 0;

        // Split the pattern into tokens
        String[] patternTokens = subCommand.value().split(" ");
        if (args.length != patternTokens.length)
            count -= 50;
        // Compare the pattern tokens with the unknown tokens
        for (int i = 0; i < patternTokens.length; i++) {
            if (isCustomVariable(patternTokens[i])) {
                count++;
                continue;
            }
            if (i < args.length && patternTokens[i].equals(args[i])) {
                count += 100;
                continue;
            }
            count -= 100;
    ...
    private boolean isCustomVariable(String in) {
        return in.startsWith("{") && in.endsWith("}");
    }

    public boolean enoughParameters(String[] args) {
        return args.length >= subCommand.value().split(" ").length;
    }
